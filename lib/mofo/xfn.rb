...
class XFN < Microformat
  ...
  class Link < OpenStruct
    def initialize(*args)
      ...
      def relation.has?(value)
        is_a?(Array) ? include?(value.to_s) : self == value.to_s
      end
    end
  ...
  def method_missing(method, *args, &block)
    if (rels = method.to_s.split(/_and_/)).size > 1
      self[*rels]
    elsif @links.respond_to? method
      @links.send(method, *args, &block)
    else
      check = args.first == true ? :== : :has?
      @links.select { |link| link.relation.send(check, method.to_s) }.first_or_self
    end
  end
end
